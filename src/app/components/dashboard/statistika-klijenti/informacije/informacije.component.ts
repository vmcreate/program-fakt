...
export class InformacijeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subKompanija = this.kompanijaService.getIzabranuKompaniju().subscribe(res => {
      ...
      this.subRacun = this.klijentService.getKlijentRacun(this.routeSub).subscribe(racuni => {
        this.racuni = [];
        this.ukupniTrosak = 0;
        this.nenaplaceno = 0;
        this.ukupniPrihod = 0
      ...
      this.subPredracun = this.klijentService.getKlijentPredracun(this.routeSub).subscribe(predracuni => {
        let arr: any = [];
        let unique: any = [];

        predracuni.map(predracun => {
          if (this.kompanijaId === predracun.payload.doc.data().kompanijaUid) {
            arr.push({ ...predracun.payload.doc.data(), id: predracun.payload.doc.data(), tip: 'Predracun' })
            this.proizvodi?.push(...predracun.payload.doc.data().proizvodi)
          }
        });
        unique = this.proizvodi?.map(proizvod => {
          return proizvod.proizvod
        })
        this.proizvodiUnique = [...new Set(unique)];
        this.racuni?.push(...arr)
