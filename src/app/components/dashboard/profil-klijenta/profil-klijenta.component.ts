...
@Component({
  selector: 'app-profil-klijenta',
  templateUrl: './profil-klijenta.component.html',
  styleUrls: ['./profil-klijenta.component.css']
})
export class ProfilKlijentaComponent implements OnInit, OnDestroy {
  dataSource?: any;
  racuni?: Array<any> = [];
  ...
  ngOnInit(): void {
    ...
    this.isLoading = true;
    this.klijentService.getLoggedKlijent().subscribe(data => { this.routeSub = data?.uid });

    setTimeout(() => {
      this.klijentService.getKlijenta(this.routeSub).subscribe((response: any) => {
  ...
  updateKlijenta(f: NgForm) {
    this.klijentService.updateKlijentaSamo(this.klijent?.id, f.value)
      .then(() => {
